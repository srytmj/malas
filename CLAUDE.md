...
| Filter tag (tree per kategori AniList) numpang di popover Filter Genre yang sama | `GenreMultiSelect.tsx` (section "Tag" di bawah section genre, kategori collapsible tapi tetap ke-render penuh di DOM biar search box di atas tetap bisa nyaring tag di kategori yang ciut) + `User\SeriesController::tagOptions()` (OR-match `whereJsonContains('tags', ...)`, sama pola dengan genre, kolom terpisah). Data tag (nama + kategori, mis. "Isekai" -> "Theme-Reincarnation") datang dari field `tags` AniList yang **sudah** di-fetch dari awal (dipakai buat `themes`/`demographics`) tapi dulu dibuang — sekarang disimpan penuh (minimal rank 60, non-spoiler) ke kolom baru `series.tags` (flat array nama, buat filter) dan `series.tag_categories` (map nama->kategori, cuma buat bikin tree, TIDAK pernah di-query) lewat `AniListController::extractAllTagNames()`/`extractTagCategoryMap()` — satu sumber data (AniList), series lama butuh re-sync ("Sync AniList") buat keisi field ini |
...
| Tiga view Koleksiku: Grid, Compact (baru), Table | `Collection/Index.tsx` — satu toggle segmented (`ViewToggle`, ikon + indikator geser CSS transform) ganti dua tombol grid/table terpisah. Grid card diperbesar (190x260) + hover overlay desktop (progress, stepper, tandai-semua, "Lanjut ke Vol X") yang cuma muncul di `md:` ke atas (`opacity-0`/`group-hover:opacity-100`, `pointer-events-none` pas ciut). Compact = row text-only dense, desktop hover swap badge→stepper+info+opsi, mobile 2-baris selalu-tampil (hover nggak ada konsepnya di touch). `RowActionsMenu` (4 item: Lihat/Lanjut ke Vol X/Tandai semua terbaca/Hapus, dropdown bukan context-menu) dipakai di grid+compact buat nutup akses aksi yang ke-skip pas mobile nggak dapet hover. Animasi pakai Tailwind/`tw-animate-css` yang udah ada (bukan Anime.js — nggak ada kebutuhan orkestrasi yang CSS transition/transform doang nggak bisa handle). Skeleton (`GridSkeleton`/`CompactSkeleton`/`TableSkeleton`) dipasang buat state `refreshing` |
...
| Export/import koleksi (backup pribadi) | `CollectionController::export()`/`import()` — export ke JSON (`response()->streamDownload()`, pola sama dengan `DatabaseBackupController`), berisi identifier series (`anilist_id`/`ranobedb_id`/`mal_id`/`slug` — dikirim semua biar robust) + condition/rating/review + full volume list. Import matching berurutan (`anilist_id` → `ranobedb_id` → `mal_id` → `slug`) terhadap katalog **lokal** — **tidak** auto-fetch dari AniList kalau series belum ada di katalog (skip + dilaporkan count-nya, bukan trigger import AniList; hindari duplikasi logic `AniListController`). Series yang sudah ada di koleksi user di-skip juga (append-only, bukan overwrite/replace). Semua field di-whitelist/sanitize (`sanitizeEnum()`) sebelum masuk DB — jangan percaya file upload user mentah-mentah. UI: tombol Export/Import di header `Collection/Index.tsx`, pola file-input dialog sama dengan `DatabaseTab` di `Admin/Settings/Index.tsx` |
| Badge "volume kurang" (next volume to buy) di Koleksiku, klik buat langsung isi form Tambah Volume | `CollectionController::missingVolumes()` — `range(1, series.total_volumes)` dikurangi nomor volume yang udah dimiliki (`CollectionVolume.volume_number`); `total_volumes` dari AniList/RanobeDB udah representasi volume yang SUDAH rilis (bukan proyeksi total), jadi nggak perlu filter tanggal rilis terpisah. Series dengan `total_volumes` null di-skip (nggak bisa dihitung). Ditampilkan sebagai badge amber `{{count}} kurang` di grid card & table row (`MissingVolumesBadge` di `Collection/Index.tsx`) — hover buka `HoverCard` nampilin nomor lengkap dikompres pakai syntax range yang sama dengan input "Tambah Volume" (mis. `4, 6-7, 9-12`, fungsi `compressRanges()`). Badge-nya `<Link>` ke `collection.show` dengan query param `?addVolumes={numbers}` — `Collection/Show.tsx` baca param itu di `useEffect` sekali pas mount, prefill field `volumes` form "Tambah Volume" + auto-buka dialognya, terus bersihin query param dari URL bar via `window.history.replaceState` (biar refresh/back nggak ke-trigger ulang) |
| Rekomendasi "Series Serupa" di halaman detail Katalog | `User\SeriesController::show()` + `Catalog/Show.tsx` — OR-match genre yang sama (`orWhereJsonContains('genres', ...)`, sama pola dengan filter genre Katalog Index), fallback ke series dengan tipe yang sama kalau series ini nggak punya genre sama sekali, `inRandomOrder()->limit(12)`, ditampilkan pakai `SeriesCard` yang sama dengan Katalog Index. **Gantiin section daftar volume yang dihapus** (lihat baris di bawah) |
| Icon mata gede jadi kontrol utama di kartu volume grid Koleksiku | `Collection/Show.tsx` — sejak cover per-volume dihapus (baris di atas), area tengah kartu grid volume yang tadinya placeholder kosong (nomor volume doang) diganti jadi tombol besar `Eye`/`EyeOff` buat toggle baca/belum (`toggleRead()`), plus label "Vol. X" kecil di bawah icon-nya — icon kecil yang sebelumnya nempel pojok kiri-atas dihapus (redundan) |

**Dihapus (bukan backlog — sengaja di-revert total, dokumentasi dipertahankan buat konteks histori):**
- **Cover per-volume (upload manual, AniList single-pick, SerpApi auto-search/bulk)** — awalnya dibangun di sesi ini (Google Books → ditinggalkan karena hasil random terbukti lewat testing → SerpApi Google Images), tapi diputuskan dihapus TOTAL setelah direfleksikan ulang: inti aplikasi ini cuma buat ngetrack progres baca & kepemilikan (fisik/digital), bukan galeri gambar per volume — cover series (level `Series`, bukan `Volume`) tetap ada dan tetap dipakai. Kolom `volumes.cover_path` di-drop lewat migration (`2026_08_30_222726_drop_cover_path_from_volumes_table.php`), `SerpApiService`/`SerpApiSettingsService`/`SerpApiSetting`/`SerpApiSettingController` dihapus filenya, tab "SerpApi" di Admin Settings dihapus, `VolumeGrid.tsx` (dipakai buat nampilin cover per volume di Katalog) dihapus total. `VolumeController::searchCovers()`/`applyCovers()` juga dihapus — method yang tersisa cuma `generate`/`store`/`edit`/`update`/`destroy`/`restore`, semua tanpa handling file cover.
...
- **Performa loading gambar cover setelah idle lama** (didiskusikan 2026-08-16, belum di-"gas") — dua perbaikan yang saling melengkapi:
  1. `loading="lazy"` di semua `<img>` cover di grid Katalog/Koleksi/Dashboard — sekarang semua gambar yang ke-render langsung di-fetch sekaligus, bukan cuma yang keliatan di viewport.
  2. `Cache-Control` buat asset gambar — beda caranya tergantung driver storage: kalau `local`, tambah `location` block di config nginx (`docs/DEPLOYMENT.md`) buat path `/storage/` & `/build/assets/` (`expires 1y; add_header Cache-Control "public, immutable";`); kalau `s3` (termasuk S3-compatible self-hosted kayak MinIO di homelab), nginx nggak kepake sama sekali karena browser fetch langsung dari S3/MinIO — perlu set `'options' => ['CacheControl' => 'public, max-age=31536000, immutable']` di `StorageSettingsService::buildS3Disk()` (`app/Services/StorageSettingsService.php:46`), otomatis kepasang ke semua upload baru. **Catatan**: opsi S3 cuma berlaku buat file yang di-upload SETELAH perubahan — cover lama di bucket nggak otomatis ke-update headernya kecuali di-reupload/copy-metadata manual.
  - Konteks homelab: user rencana simpen media di disk fisik terpisah lewat MinIO (S3-compatible), bukan folder lokal — app ini udah support endpoint S3-compatible custom dari awal (`buildS3Disk()` nerima `endpoint` custom), jadi nggak perlu ubah kode buat itu, tinggal isi endpoint MinIO di `/admin/settings/storage`.
